...
\subsection{Notation}
The following indices are used. The sample index is denoted $k$
...
It is assumed that load factors $\mbf{n}_c$ are tracked by fast inner loop controllers in the vehicle body frame.
...
Considering two UAS $k$ and $k-1$, this leads to relative position error dynamics

\begin{align}
\Delta \mbf{p} &= \mbf{p}_k - \mbf{p}_{k-1} - \Delta \mbf{p}_c \\
\Delta \dot{\mbf{p}} &= \mbf{v}_{k} - \mbf{v}_{k-1} -  \Delta \dot{\mbf{p}}_c\\
\Delta \dot{\mbf{v}} &= \mbf{a}_{k} - \mbf{a}_{k-1} -  \Delta \ddot{\mbf{p}}_c\\
{} &= \mbf{a}_{c,k} + \mbf{a}_{w,k}
 - \mbf{a}_{k-1} -  \Delta \ddot{\mbf{p}}_c\\
{} &= \mbf{u} + \mbf{a}_{w,k}
 - \mbf{a}_{k-1} -  \Delta \ddot{\mbf{p}}_c
\end{align}

where $\Delta \mbf{p}$ is the relative position error between UAS $k$ and its predecessor $k-1$, $\Delta \mbf{v}$ is the corresponding relative velocity error, $\Delta \mbf{p}_c$ is the desired relative position to the predecessor, $\mbf{a}$ are accelerations.
...
\begin{align}
\mbf{\Phi}_u &= \mbf{a}_{w,k} - \mbf{a}_{k-1}
\end{align}
...
Both controllers have been evaluated in a simulation environment. In all cases, the vehicle dynamics have been integrating with a Euler scheme at a sampling time of $t_{sim} = 10^{-4} \, s$, while the controller sampling time has been varied.

\subsection{Controller parameters}

For the CSTSMC, the controller gains are computed according to (\ref{eq:csmcgaincondition}). 

The unknown perturbations $\Phi_{u,i}$ are driven by exogenous disturbances $a_{w,i}$ and acceleration of the predecessor $a_{k-1,i}$.

Those being bounded by

\begin{align}
|a_{w,i}| &\leq A_{w,i} \\
|a_{k-1,i}| &\leq A_{k-1,i}
\end{align}

To obtain the bounds on

For the DSMC, the entries of the reaching rate matrix $\mbf{\Psi}$ are selected to stay within control input limits for typical initial position errors.
